...
import com.handmark.pulltorefresh.library.internal.RedHeartLoadingLayout;
...
//    	TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefresh);
//    	return new RotateLoadingLayout(context, Mode.PULL_FROM_START, getPullToRefreshScrollDirection(), a);
    	TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefresh);
    	return new RedHeartLoadingLayout(context, Mode.PULL_FROM_START, getPullToRefreshScrollDirection(), a);
    }
    
    /**
     * 进入界面先显示下拉刷新，纯假象。。。暂定如此
     */
    public void showhead(){
    	...
    	final int newScrollValue;
		final int itemDimension;
//		itemDimension = mHeaderLayout.getContentSize();
		int dps = 50+2*12;
		itemDimension = DensityUtil.dip2px(mContext, dps);
//    	itemDimension = 116;
    	scrollTo(0, -itemDimension);
    	float scale = 1.0f;
    	mHeaderLayout.onPull(scale);
...
//		smoothScrollTo(0);
     ...
     * @param top_padding
     */
    public void setHeadPaddingTop(float top_padding){
    	if(mHeaderLayout !=null&&mHeaderLayout.mInnerLayout != null){
    		mHeaderLayout.mInnerLayout.setPadding(DensityUtil.dip2px(mContext, 24),
    				DensityUtil.dip2px(mContext, top_padding), 
    				DensityUtil.dip2px(mContext,24), 
    				DensityUtil.dip2px(mContext, 12));
    	}
    }
}
