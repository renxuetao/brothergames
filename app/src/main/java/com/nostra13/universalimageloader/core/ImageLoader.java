...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		if(uri.length() > 0 && !uri.contains("http") && !uri.contains("file:///") && !uri.contains("drawable://")
				&& !uri.contains("content://") && !uri.contains("assets://")){
//			Srv srv = MyApplication.getInstance().interaction.ImageSrv;
			...
			if(uri.length() > 1 && uri.substring(0, 1).equals("/")){
				uri = uri.substring(1, uri.length());
			}
			uri = "http://"+ser_ip+":"+ser_port+"/"+uri;
		}
		checkConfiguration();
		if (imageAware == null) {
	...
	public Bitmap isHaveImage(String uri, ImageAware imageAware) {
		if(uri.length() > 0 && !uri.contains("http") && !uri.contains("file:///") && !uri.contains("drawable://")
				&& !uri.contains("content://") && !uri.contains("assets://")){
//			Srv srv = MyApplication.getInstance().interaction.ImageSrv;
			String ser_ip = MyApplication.getInstance().preferences.getString("ImageSrv", "");
